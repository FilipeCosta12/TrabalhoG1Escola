...
export class AppComponent implements OnInit {
    ...
    cadastrarTurma(): void {
        this.turmas.push(new Turma(this.turma_atual.nome));
        this.turma_atual = new Turma(null);
    }

    limparTurma(): void{
        this.turma_atual.nome = null;
    }

    excluirTurma(turma: Turma): void{
        for (let i = 0; i < this.turmas.length; i++){
            if (this.turmas[i].nome == turma.nome) {
                this.turmas.splice(i, 1);
                this.excluirAlunosTurma(turma.nome);
                break;
            }
        }
    }
    ...
    visualizarTurma(nomeTurma: number): void{
